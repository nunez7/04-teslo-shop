...
export const paypalCheckPayment = async (paypalTransactionId: string) => {
    ...
    //Si se hace el response de la verificación, actualizamos la BD
...
//Metodo post para obtener el access token y consultar el detalle del pago a verifyPayPalPayment
const getPayPalBearerToken = async (): Promise<string | null> => {
    const PAYPAL_CLIENT_ID = process.env.NEXT_PUBLIC_PAYPAL_CLIENT_ID;
    const PAYPAL_SECRET = process.env.PAYPAL_SECRET;
    const oauth2Url = process.env.PAYPAL_OAUTH_URL ?? "";

    //Codificando las claves para que vallan encriptadas al token
...
//Consultar el detalle del pago mediante el token y transactionId, metodo GET
